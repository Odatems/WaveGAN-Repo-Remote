...
class GurobiTSPReader(object):
    ...
    def process_batch(self, lines):
        ...
        batch_tour_nodes = []
        batch_tour_len = []
        W_val = np.zeros(( self.num_nodes, self.num_nodes))
        ...
        for line_num, line in enumerate(lines):
            ...
            counter = 0
            nodes_comb = [node_id for node_id in range(self.num_nodes)]
            for tx,rx in combinations(nodes_comb,2):
                W_val[tx,rx]  = line[counter]
                W_val[rx,tx] = line[counter] # for symmetry 
                counter=counter+2
