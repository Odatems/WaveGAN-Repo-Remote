...
# This block of code is added as reported in one of the papers about bug in the code
def is_valid(tour):
    nodes = set(tour)
    for n in nodes:
        if tour.count(n) != 1:
...
def beamsearch_tour_nodes(y_pred_edges, beam_size, batch_size, num_nodes, dtypeFloat, dtypeLong, probs_type='raw', random_start=False):
    ...
    if probs_type == 'raw':
        ...
        y = F.log_softmax(y_pred_edges, dim=3) # B x V x V x voc_edges
        #print('y before values are = ', y.shape)
        ...
        #print('After ....y is equal to = ', y)
    # Perform beamsearch
    beamsearch = Beamsearch(beam_size, batch_size, num_nodes, dtypeFloat, dtypeLong, probs_type, random_start)
    print('hello; this is the start of debugging the code ')
    trans_probs = y.gather(1, beamsearch.get_current_state().type(torch.int64))#
    #print('current state is = ', beamsearch.get_current_state().type(torch.int64))
    #print('trans prob = ', trans_probs)
    
    #print('indicies are = ', beamsearch.get_current_state().type(torch.int64))
    #print('transition probability  = ', trans_probs)
    print('***************************************************************')
    print('we are in the node iteration loop.......')
    for step in range(num_nodes - 1):
        
        #print('iteration number = ', step)
        beamsearch.advance(trans_probs)
        #print('indicies are = ', beamsearch.get_current_state().type(torch.int64))
        trans_probs = y.gather(1, beamsearch.get_current_state().type(torch.int64))#.type(torch.int64)
        #print('transition probability  = ', trans_probs)
        #print('current state is = ', beamsearch.get_current_state().type(torch.int64))
        #print('trans prob = ', trans_probs)
        print('==========================================================')
...
def beamsearch_tour_nodes_shortest(y_pred_edges, x_edges_values, beam_size, batch_size, num_nodes,
    ...
    # This peice of code is added in response to the error reported in one of the papers about TSP .
    #GRAPH NEURAL NETWORK GUIDED LOCAL SEARCH FOR THE TRAVELING SALESPERSON PROBLEM
    shortest_tours_list = shortest_tours.cpu().numpy().tolist()
    #print('\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\....')
    tour_is_valid = np.array([is_valid(tour) for tour in shortest_tours_list])
    '''
    if not tour_is_valid.all():
        print('This is invalid tour....')
        #print(shortest_tours, tour_is_valid)
    else:
        print('This is a valid tour')
